...
local function setupQuestTemplateV2(quest, questTemp)
	local titleText = nil
	...
	local claim = questTemp:FindFirstChild("Claim") or questTemp:FindFirstChild("ClaimButton")
	local claimButton = nil
	if claim then
		if claim:IsA("GuiButton") then
			claimButton = claim
		else
			claimButton = claim:FindFirstChild("Btn", true) or claim:FindFirstChildWhichIsA("GuiButton", true)
		end
	end

	local statusLabel = claim and (claim:FindFirstChild("TextLabel", true) or claim:FindFirstChild("Status", true))
	...
	local function setClaimState(state)
		canClaimReward = state == "Claimable"

		if claim then
			claim.Visible = canClaimReward
		end
	...
	local function updateProgressBar(currentProgress, progressGoal)
		pcall(function()
			...
			if rewardsBar and rewardFolder then
				setupRewardDisplay(rewardFolder, rewardsBar)
			end
			...
			if progressTextLabel then
				progressTextLabel.Text = ("PROGRESS %d/%d"):format(progressValue, goalValue)
			end

			local questIsCompleted = (completed and completed.Value) or (progressGoal and goalValue > 0 and progressValue >= goalValue)
			if claimed and claimed.Value then
				setClaimState("Claimed")
			elseif questIsCompleted then
				setClaimState("Claimable")
			else
				setClaimState("Incomplete")
			end
		end)
	end
	...
	if progress and goal then
		...
		if claimed then
			claimed.Changed:Connect(function()
				updateProgressBar(progress, goal)
			end)
		end
	else
		local part, partProgress, partGoal, partClaimed, partCompleted = nil, nil, nil, nil, nil
		local partConnections = {}
		local updatePartInformation

		local function clearPartConnections()
		...
		local function hookPartConnections()
			clearPartConnections()

			if partProgress then
				table.insert(partConnections, partProgress.Changed:Connect(updatePartInformation))
			end
			if partGoal then
				table.insert(partConnections, partGoal.Changed:Connect(updatePartInformation))
			end
			if partClaimed then
				table.insert(partConnections, partClaimed.Changed:Connect(updatePartInformation))
			end
			if partCompleted then
				table.insert(partConnections, partCompleted.Changed:Connect(updatePartInformation))
			end
		end

		function updatePartInformation()
			local success, err = pcall(function()
				...
				partCompleted = part:FindFirstChild("Completed")
				...
				if titleText and titleQuestLabel then
					titleQuestLabel.Text = titleText
				end
				...
				local partIsCompleted = (partCompleted and partCompleted.Value)
					or (partProgress and partGoal and partGoal.Value > 0 and partProgress.Value >= partGoal.Value)

				if partClaimed and partClaimed.Value then
					setClaimState("Claimed")
				elseif partIsCompleted then
					setClaimState("Claimable")
				else
					setClaimState("Incomplete")
				end
			...
			if not success then
				warn("[Quests] Failed to update quest part UI:", err)
			end
		end
	...
	if description and descLabel then
		descLabel.Text = description.Value
	end
	...
	if claimButton and claimButton:IsA("GuiButton") then
		claimButton.Activated:Connect(claimQuest)
	end
...
local function renderQuests(category)
	...
	local function spawnQuests(folderName)
		...
		for _, quest in ipairs(questsFolder:GetChildren()) do
			...
			setupQuestTemplateV2(quest, questTemp)

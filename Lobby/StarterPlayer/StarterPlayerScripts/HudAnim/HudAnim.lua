...
local function is_animation_disabled(inst: Instance): boolean
	local current: Instance? = inst
	while current do
		if current.Name == "RewardPopUp"
			or current:GetAttribute("NoAnimation") == true
			or current:GetAttribute("NoHudAnimation") == true then
			return true
		end
...
function HudAnim.apply_defaults_to_buttons(root: Instance, extra: {}?): ()
	for _, d in root:GetDescendants() do
		if d:IsA("GuiButton") and not is_animation_disabled(d) then
...
function HudAnim.bind(inst: GuiObject): ()
	...
	if is_animation_disabled(inst) then
		return
	end
